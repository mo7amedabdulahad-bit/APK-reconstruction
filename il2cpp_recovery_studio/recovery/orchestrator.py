...
class RecoveryOrchestrator:
    ...
    def run(self, apk_info: APKInfo, output_dir: Optional[Path] = None) -> PipelineResult:
        ...
        if dumper_result.success or dumper_result.status == ToolStatus.PARTIAL_SUCCESS:
            # Parse output on both clean and partial success — script.json
            # may be usable even when the tool exited with a non-zero code.
            if dumper_result.status == ToolStatus.PARTIAL_SUCCESS:
                logger.warning(
                    "Il2CppDumper: parsing partial output — "
                    "downstream fidelity may be reduced"
                )
    ...
    def _run_il2cpp_dumper(self, apk_info: APKInfo, output_dir: Path) -> ToolResult:
        ...
        After the generic subprocess execution, we inspect the output
        directory for ``script.json``.  If it exists despite a non-zero
        exit code, the result is promoted to ``PARTIAL_SUCCESS`` so that
        downstream consumers know usable output was produced.
        """
        ...
        result = self._execute_tool("Il2CppDumper", cmd, output_dir)
        ...
        has_readkey_crash = any(
            m.lower() in stdout_lower
            for m in ["readkey", "console.readkey", "system.invalidoperationexception", "press any key"]
        )
        has_protected = any(
            m.lower() in stdout_lower
            for m in ["protected", "access denied", "unauthorizedaccess"]
        )

        if result.status == ToolStatus.SUCCESS and has_script:
            # Clean success — nothing to change
            pass
        elif result.status == ToolStatus.FAILED and has_script:
            # Non-zero exit but script.json exists → partial success
            ...
            result.error_message = (
                f"Partial success — script.json recovered ({'; '.join(parts)}). "
                "Downstream scene fidelity may be reduced."
            )
            logger.warning("Il2CppDumper partial success: %s", result.error_message)
        elif not has_script:
            # Hard failure — no usable output
            result.error_message = (
                f"Hard failure — no script.json produced (exit_code={result.return_code}). "
                "MonoBehaviours will fall back to raw bytes. "
                "Scene reconstruction fidelity is significantly reduced."
            )
            logger.warning("Il2CppDumper hard failure: %s", result.error_message)
