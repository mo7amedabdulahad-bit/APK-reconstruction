#!/usr/bin/env python3
"""
IL2CPP Recovery Studio — app.py v12

Changes vs v11:
- Manual Java picker button added to the left panel.
  User can browse to java.exe; path is saved to tools/config.json and
  reloaded on next launch.
- Wider automatic Java search: Zulu, BellSoft Liberica, Corretto, Semeru,
  Scoop shims, Chocolatey, and LOCALAPPDATA/Programs are now checked.
- Java detection also walks HKCU registry hive (not just HKLM).
- _find_java() accepts optional override from stored config.
- apktool .bat is always (re)written with the located java path.
"""
...
class IL2CPPStudio(ctk.CTk):
    ...
    def _do_report(self, out: Path):
        ...
        html = (
            ...
            f"<hr><p style='color:#5c5c8a'>Generated by IL2CPP Recovery Studio v12</p>"
            "</body></html>"
        )
        rp = out / "report.html"
