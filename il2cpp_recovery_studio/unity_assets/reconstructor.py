...
# Assets with confidence at or below this threshold are skipped when
# APKREC_ENABLE_LOW_CONFIDENCE_ASSET_COPY is disabled.
    ...
    def _build_project_structure(self, result: ReconstructionResult,
        ...
        for feature, subcats in feature_groups.items():
            ...
            for subcat, assets in subcats.items():
                ...
                for asset in assets:
                    if not ff.enable_low_confidence_asset_copy and asset.confidence <= _LOW_CONFIDENCE_THRESHOLD:
                        skipped_low_conf += 1
                        continue
        ...
        if skipped_low_conf:
            logger.info("[FEATURE] Skipped %d low-confidence assets (threshold=%.1f)",
                        skipped_low_conf, _LOW_CONFIDENCE_THRESHOLD)
