...
* Neon cyan / purple / green accents with glow effect
* Animated progress bar & pulsing buttons
* Wizard flow: Drop file → Choose operations → Auto-run → Results
"""
...
class NeonButton(ctk.CTkButton):
    """Animated neon button with pulse effect."""
...
class LogConsole(ctk.CTkTextbox):
    """Dark terminal-style scrolling log."""
...
class IL2CPPStudio(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Operation booleans
        self._ops: dict[str, ctk.BooleanVar] = {
    ...
    def _build_left(self, parent):
        ...
        ops_def = [
            ...
            ("extract_il2cpp",  "Parse IL2CPP metadata",          "Recovers class / method names"),
        ...
        self._open_btn = NeonButton(
            left, "  📁  Open Output Folder",
    ...
    def _animate_header(self):
        """Subtle scanline flicker on the header."""
    ...
    def _animate_progress(self):
        ...
        # Slowly creep to 0.95 while running; jump to 1.0 when done
        if cur < 0.95:
    ...
    def _browse_apk(self):
        ...
        if path:
            ...
            name = self._apk_path.name
            self._drop_lbl.configure(text=f"✅  {name}", text_color=NEON_GREEN)
            self._file_lbl.configure(text=str(self._apk_path), text_color=NEON_GREEN)
            self._status(f"File loaded: {name}")
    ...
    def _start_analysis(self):
        ...
        thread = threading.Thread(target=self._run_pipeline, args=(ops,), daemon=True)
        thread.start()

    def _run_pipeline(self, ops: dict[str, bool]):
        ...
        try:
            ...
            if ops["extract_assets"]:
                ...
                self._step("Extracting Unity assets (UnityPy)...", 0.35)
            ...
            # ── Step 3: IL2CPP metadata ───────────────────────────
            if ops["extract_il2cpp"]:
                meta_dir = out / "il2cpp_meta"
                self._step("Parsing IL2CPP metadata...", 0.55)
            ...
            # ── Step 4: Smali decompile ───────────────────────────
            if ops["decompile_smali"]:
                smali_dir = out / "smali"
                self._step("Decompiling to Smali...", 0.70)
            ...
            # ── Step 5: HTML report ───────────────────────────────
            if ops["gen_report"]:
                self._step("Generating HTML report...", 0.85)
    ...
    def _parse_il2cpp(self, raw_dir: Path, out: Path):
        """Lightweight il2cpp metadata parser — looks for global-metadata.dat."""
        ...
        if not meta_files:
            ...
            return
        # Copy the files so downstream tools can process them
    ...
    def _run_apktool(self, apk: Path, out: Path):
        ...
        if not apktool:
            ...
            return
        self._log_main(f"Running apktool on {apk.name}...", "INFO")
        result = subprocess.run(
            [apktool, "d", str(apk), "-o", str(out), "--force"],
    ...
    def _generate_report(self, out: Path):
        html = self._build_report_html(out)
        report_path = out / "report.html"
        report_path.write_text(html, encoding="utf-8")
        self._log_main(f"HTML report saved: {report_path}", "OK")

    def _build_report_html(self, out: Path) -> str:
...
<style>
body{{background:#0a0a0f;color:#e0e0ff;font-family:'Courier New',monospace;padding:30px}}
h1{{color:#00ffe7;border-bottom:1px solid #a259ff;padding-bottom:8px}}
table{{border-collapse:collapse;width:100%}}
td{{padding:4px 12px;border-bottom:1px solid #1a1a40}}
th{{color:#a259ff;text-align:left;padding:6px 12px}}
</style></head><body>
<h1>⬡ IL2CPP Recovery Studio Report</h1>
<p>Output directory: <code>{out}</code></p>
<h2 style='color:#39ff14'>Folders</h2>
<table><tr><th></th><th>Name</th></tr>{dir_rows}</table>
<h2 style='color:#39ff14'>Files</h2>
<table><tr><th></th><th>Name</th></tr>{file_rows}</table>
<hr><p style='color:#5c5c8a'>Generated by IL2CPP Recovery Studio v2</p>
    ...
    def _show_results(self, out: Path):
        ...
        lines += ["", "  NEXT STEPS:",
                  ...
                  "  2. Inspect unity_assets/ for textures",
                  "  3. Use Il2CppDumper on il2cpp_meta/",
                  "  4. Click 'Open Output Folder' above"]
        self._set_results("\n".join(lines))
    ...
    # ── Helpers ───────────────────────────────────────────────────
    def _step(self, msg: str, progress: float):
