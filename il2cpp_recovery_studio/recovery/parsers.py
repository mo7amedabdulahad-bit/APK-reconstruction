...
class Il2CppDumperParser:
    ...
    When *degraded* is True (partial success — script.json recovered
    despite non-zero exit), parsed classes are tagged with reduced
    confidence so downstream consumers do not treat them as
    ...
    _CONFIDENCE_DEGRADED = 0.6

    def parse(self, output_dir: Path, degraded: bool = False) -> list[RecoveredClass]:
        classes: list[RecoveredClass] = []
        if not output_dir.is_dir():
            logger.warning("Il2CppDumper output dir not found: %s", output_dir)
            return classes

        confidence = self._CONFIDENCE_DEGRADED if degraded else self._CONFIDENCE_CLEAN
        if degraded:
            logger.warning(
                "Il2CppDumper: parsing in DEGRADED mode — "
                "confidence reduced to %.1f", confidence
            )

        # Parse script.json (primary structured output)
        script_json = output_dir / "script.json"
        if script_json.exists():
            classes.extend(self._parse_script_json(script_json, confidence))

        # Parse dump.cs (human-readable dump)
        dump_cs = output_dir / "dump.cs"
        if dump_cs.exists():
            classes.extend(self._parse_dump_cs(dump_cs, confidence))
        ...
        logger.info(
            "Il2CppDumper: recovered %d classes from %s (degraded=%s)",
            len(classes), output_dir, degraded,
        )
        return classes

    def _parse_script_json(self, path: Path, confidence: float = 0.9) -> list[RecoveredClass]:
        classes = []
        try:
            ...
            for type_entry in data:
                cls = self._parse_script_type(type_entry, confidence)
                if cls:
                    classes.append(cls)
        except (json.JSONDecodeError, OSError) as exc:
    ...
    def _parse_script_type(self, entry: dict, confidence: float = 0.9) -> Optional[RecoveredClass]:
    ...
    def _parse_dump_cs(self, path: Path, confidence: float = 0.7) -> list[RecoveredClass]:
        classes = []
        try:
            content = path.read_text(encoding="utf-8", errors="replace")
