...
class NativeAnalyzer:
    ...
    @staticmethod
    def _basic_format_detect(binary_path: str) -> BinaryInfo:
        """Best-effort format detection from magic bytes (no LIEF required)."""
        info = BinaryInfo(file_path=binary_path, file_size=Path(binary_path).stat().st_size)
        try:
        ...
        if header[:4] == ELF_MAGIC:
            ...
            # ELF header byte 4: 1=32-bit, 2=64-bit
            if len(header) > 4:
                info.bits = 64 if header[4] == 2 else 32
            # ELF header byte 5: 1=LE, 2=BE
            if len(header) > 5:
                info.endian = "little" if header[5] == 1 else "big"
            # ELF header byte 18: e_machine (2 bytes, little-endian)
            if len(header) >= 20:
        ...
        elif header[:4] in (MACHO_MAGIC_32, MACHO_MAGIC_64) or header[:4] == MACHO_FAT_MAGIC:
    ...
    def analyze(
        ...
        if not HAS_LIEF:
            ...
            result.binary_info = self._basic_format_detect(binary_path)
            return result
