...
from il2cpp_recovery_studio.core.diagnostics import StartupDiagnostics
...
IL2CPPDUMPER_VER      = "v39-fork (Windows81)"
...
def _write_il2cpp_config(exe_dir: Path, log):
    ...
    Skip if the v39-fork version is detected (it natively supports metadata v39
    and its own config should not be overwritten with ForceVersion=31).
    """
    # The v39 fork has its own deps.json/runtimeconfig.json — don't overwrite its config
    if (exe_dir / "Il2CppDumper.deps.json").exists():
        log(f"[INFO ] Il2CppDumper v39 fork detected — keeping native config")
        return
            ...
            try:
                ...
                if img:
                    ...
                    if seen is not None:
                        ...
                        if h in seen:
                            continue  # duplicate, don't count as skipped
                        seen.add(h)
...
def _classify_dumper_output(
    proc_returncode: int,
    stdout_data: str,
    has_script_json: bool,
    has_dlls: bool,
) -> tuple[Il2CppDumperStatus, str]:
    """Classify Il2CppDumper run outcome into a granular status.

    Returns (status, error_message).
    """
    has_readkey_crash = any(m in stdout_data for m in _READKEY_CRASH_MARKERS)
    has_protected = any(m in stdout_data for m in _PROTECTED_FILE_MARKERS)

    if proc_returncode == 0 and has_script_json:
        return Il2CppDumperStatus.CLEAN_SUCCESS, ""

    if has_script_json:
        parts = [f"exit_code={proc_returncode}"]
        if has_readkey_crash:
            parts.append("ReadKey crash detected")
        if has_protected:
            parts.append("protected-file warnings")
        return (
    ...
    return (
        Il2CppDumperStatus.HARD_FAILURE_NO_SCRIPT_JSON,
        f"hard failure — exit_code={proc_returncode}, no script.json",
    )
...
def _run_stage4a_il2cppdumper(
    ...
    """Run Il2CppDumper and return a structured result.

    The result distinguishes between clean success, partial success
    (script.json exists but non-zero exit), and hard failure so that
    downstream stages can adjust their confidence accordingly.
    """
    from il2cpp_recovery_studio.recovery.models import (
        Il2CppDumperRunResult,
        Il2CppDumperStatus,
    ...
    if script_json.exists() and dll_dir.exists() and any(dll_dir.glob("*.dll")):
        ...
        return Il2CppDumperRunResult(
            status=Il2CppDumperStatus.CLEAN_SUCCESS,
            dump_dir=dump_dir,
            return_code=0,
            dll_count=dll_count,
    ...
    if not so_file or not meta_file.exists():
        ...
        return Il2CppDumperRunResult(
            status=Il2CppDumperStatus.SKIPPED,
            error_message="missing libil2cpp.so or global-metadata.dat",
        )
    ...
    if exe is None:
        ...
        return Il2CppDumperRunResult(
            status=Il2CppDumperStatus.SKIPPED,
            error_message="Il2CppDumper executable not found",
        )
    ...
    try:
        ...
        # Log every line of Il2CppDumper output
        ...
        has_script = script_json.exists()

        status, error_msg = _classify_dumper_output(
            proc.returncode, stdout_data, has_script, dll_count > 0,
        )

        result = Il2CppDumperRunResult(
            status=status,
            dump_dir=dump_dir if has_script else None,
            return_code=proc.returncode,
            stdout=stdout_data,
            error_message=error_msg,
            dll_count=dll_count,
            has_script_json=has_script,
            has_readkey_crash=any(m in stdout_data for m in _READKEY_CRASH_MARKERS),
            has_protected_warnings=any(m in stdout_data for m in _PROTECTED_FILE_MARKERS),
        )

        # ── Emit structured status ──
        result.log_status(log)

        if status == Il2CppDumperStatus.CLEAN_SUCCESS:
            log(f"[OK   ] Stage 4a complete — {dll_count} DummyDlls + script.json -> {dump_dir}")
        elif status == Il2CppDumperStatus.PARTIAL_SUCCESS_SCRIPT_JSON:
            log(f"[WARN ] Stage 4a partial — {dll_count} DummyDlls + script.json -> {dump_dir}")
        else:
            log("[FAIL] Stage 4a — no usable output produced")
...
def _parse_ui_component_fields(raw_bytes, class_name, field_start,
    ...
    try:
        if class_name == "Image":
            # Unity Image component layout (after MonoBehaviour base fields):
            #   +0   m_Material  PPtr (12 bytes)
            #   +12  m_Color     Color (16 bytes = 4 floats)
            #   +28  m_RaycastTarget bool (1 byte)
            #   +32  m_Sprite    PPtr (12 bytes)
            # Verified by m_Color.g consistently reading as 0x00000001
            # (= m_RaycastTarget true) when using old offset of +24.
            mat_off = field_start
            color_off = field_start + 12
            sprite_off = field_start + 32
            if sprite_off + 12 <= len(raw_bytes):
                fid = struct.unpack_from("<i", raw_bytes, sprite_off)[0]
                pid = struct.unpack_from("<q", raw_bytes, sprite_off + 4)[0]
                sp = _resolve_pptr_fields(pid, fid, sprite_index, source_file)
                if sp:
                    out["m_Sprite"] = sp
            if color_off + 16 <= len(raw_bytes):
                r, g, b, a = struct.unpack_from("<4f", raw_bytes, color_off)
                out["m_Color"] = {"r": r, "g": g, "b": b, "a": a}

        elif class_name == "RawImage":
            # Same layout as Image: m_Material(+0), m_Color(+12),
            # m_RaycastTarget(+28), m_Texture(+32).
            tex_off = field_start + 32
            if tex_off + 12 <= len(raw_bytes):
                fid = struct.unpack_from("<i", raw_bytes, tex_off)[0]
                pid = struct.unpack_from("<q", raw_bytes, tex_off + 4)[0]
...
# 1b. Configure TypeTreeGenerator using DummyDlls (optional enhancement)
    ...
    #     GATED: controlled by APKREC_ENABLE_ADDRESSABLE_ENRICHMENT env var.
    if feature_flags is None:
        from il2cpp_recovery_studio.core.config import FeatureFlags
        feature_flags = FeatureFlags()
    if not feature_flags.enable_addressable_enrichment:
        log("[FEATURE] Addressables catalog: skipped (feature disabled)")
    else:
        try:
...
def _run_pipeline(
    ...
    try:
        ...
        # ── Feature flags (read from environment at pipeline start) ──
        from il2cpp_recovery_studio.core.config import FeatureFlags
        feature_flags = FeatureFlags()
        feature_flags.log_state(log)
        ...
        # Shared content-hash set so identical images are deduplicated silently.
        ...
        # GATED: controlled by APKREC_ENABLE_ADDRESSABLE_ENRICHMENT env var.
        if not feature_flags.enable_addressable_enrichment:
            log("[FEATURE] Addressables catalog export: skipped (feature disabled)")
        else:
            catalog_bin = None
            ...
            if catalog_bin:
                try:
                    ...
                    out_catalog = {
                        "locator_id": catalog.LocatorId,
                        "build_result_hash": catalog.BuildResultHash,
                        "resources": {}
                    }
                    for key, locations in catalog.Resources.items():
                        ...
                        for loc in locations:
                            loc_dict = {
                                "primary_key": loc.PrimaryKey,
                                "internal_id": loc.InternalId,
                                "provider_id": loc.ProviderId,
                                "type": loc.Type.ClassName if loc.Type else None,
                                "dependencies": [dep.PrimaryKey for dep in loc.Dependencies] if loc.Dependencies else []
                            }
        ...
        dumper_result = _run_stage4a_il2cppdumper(
            il2cpp_dir, out_dir, log, manual_exe_path=il2cpp_exe_path
        )
        dump_dir = dumper_result.dump_dir

        # Warn if downstream fidelity is reduced
        if not dumper_result.success:
            fidelity_warn = dumper_result.fidelity_warning()
            if fidelity_warn:
                log(f"[WARN ] {fidelity_warn}")
        ...
        run_ui_compiler(out_dir, log, raw_dir=raw_dir, dumper_result=dumper_result)
        ...
        # Il2CppDumper status — show all three states explicitly
        _d_status = dumper_result.status.value if dumper_result else "SKIPPED"
        if dumper_result and dumper_result.status == Il2CppDumperStatus.CLEAN_SUCCESS:
            log(f"[INFO ] ✔ Stage 4a — Il2CppDumper CLEAN_SUCCESS "
                f"({dumper_result.dll_count} DummyDlls + script.json)")
        elif dumper_result and dumper_result.status == Il2CppDumperStatus.PARTIAL_SUCCESS_SCRIPT_JSON:
            log(f"[INFO ] ⚠ Stage 4a — Il2CppDumper PARTIAL_SUCCESS_SCRIPT_JSON "
                f"({dumper_result.dll_count} DummyDlls — reduced fidelity)")
        else:
            log(f"[INFO ] ✘ Stage 4a — Il2CppDumper {_d_status}")
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.after(200, self._run_startup_diagnostics)
    ...
    def _run_startup_diagnostics(self):
        """Run dependency checks and display structured summary in log box."""
        diag = StartupDiagnostics()
        diag.run()
        for line in diag.format_summary():
            self._log_box.insert("end", line + "\n")
        self._log_box.see("end")
        # Update status bar if blocking deps are missing
        if diag.has_blocking:
            self._status.configure(
                text="⚠ Missing required dependencies — see diagnostics above.",
                text_color=NEON_PINK,
            )
        elif diag.has_degraded:
            self._status.configure(
                text="⚠ Some optional features unavailable — see diagnostics above.",
                text_color=NEON_YEL,
            )
